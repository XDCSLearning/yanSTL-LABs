...
case_t allocator_traits_types()
{
    ...
    co_yield std::format("test with T = {}", typeid(T).name());
    co_yield{ std::is_same_v<typename T::value_type, int>,
               std::format("T::value_type should be int, but is {}", typeid(typename T::value_type).name()) };
    co_yield{ std::is_same_v<typename T::pointer, int*>,
               std::format("T::pointer should be int*, but is {}", typeid(typename T::pointer).name()) };
    co_yield{ std::is_same_v<typename T::const_pointer, const int*>,
               std::format("T::const_pointer should be const int*, but is {}", typeid(typename T::const_pointer).name()) };
    co_yield{ std::is_same_v<typename T::void_pointer, void*>,
               std::format("T::void_pointer should be void*, but is {}", typeid(typename T::void_pointer).name()) };
    co_yield{ std::is_same_v<typename T::const_void_pointer, const void*>,
               std::format("T::const_void_pointer should be const void*, but is {}", typeid(typename T::const_void_pointer).name()) };
    co_yield{ std::is_same_v<typename T::difference_type, std::ptrdiff_t>,
               std::format("T::difference_type should be std::ptrdiff_t, but is {}", typeid(typename T::difference_type).name()) };
    co_yield{ std::is_same_v<typename T::size_type, std::size_t>,
               std::format("T::size_type should be std::size_t, but is {}", typeid(typename T::size_type).name()) };
    co_yield{ std::is_same_v<typename T::propagate_on_container_copy_assignment, std::false_type>,
               std::format("T::propagate_on_container_copy_assignment should be std::false_type, but is {}", typeid(typename T::propagate_on_container_copy_assignment).name()) };
    co_yield{ std::is_same_v<typename T::propagate_on_container_move_assignment, std::true_type>,
               std::format("T::propagate_on_container_move_assignment should be std::true_type, but is {}", typeid(typename T::propagate_on_container_move_assignment).name()) };
    co_yield{ std::is_same_v<typename T::propagate_on_container_swap, std::false_type>,
               std::format("T::propagate_on_container_swap should be std::false_type, but is {}", typeid(typename T::propagate_on_container_swap).name()) };
    co_yield{ std::is_same_v<typename T::is_always_equal, std::true_type>,
               std::format("T::is_always_equal should be std::true_type, but is {}", typeid(typename T::is_always_equal).name()) };
    co_yield{ std::is_same_v<typename T::template rebind_alloc<double>, std::allocator<double>>,
               std::format("T::rebind_alloc<double> should be std::allocator<double>, but is {}", typeid(typename T::template rebind_alloc<double>).name()) };
    co_yield{ std::is_same_v<typename T::template rebind_traits<double>, NAMESPACE_MY allocator_traits<std::allocator<double>>>,
               std::format("T::rebind_traits<double> should be std::allocator_traits<std::allocator<double>>, but is {}", typeid(typename T::template rebind_traits<double>).name()) };
    co_yield nullptr;
    using U = std::allocator_traits<NAMESPACE_MY allocator<int>>;
    co_yield std::format("test with U = {}", typeid(U).name());
    co_yield{ std::is_same_v<typename U::value_type, int>,
               std::format("U::value_type should be int, but is {}", typeid(typename U::value_type).name()) };
    co_yield{ std::is_same_v<typename U::pointer, int*>,
               std::format("U::pointer should be int*, but is {}", typeid(typename U::pointer).name()) };
    co_yield{ std::is_same_v<typename U::const_pointer, const int*>,
               std::format("U::const_pointer should be const int*, but is {}", typeid(typename U::const_pointer).name()) };
    co_yield{ std::is_same_v<typename U::void_pointer, void*>,
               std::format("U::void_pointer should be void*, but is {}", typeid(typename U::void_pointer).name()) };
    co_yield{ std::is_same_v<typename U::const_void_pointer, const void*>,
               std::format("U::const_void_pointer should be const void*, but is {}", typeid(typename U::const_void_pointer).name()) };
    co_yield{ std::is_same_v<typename U::difference_type, std::ptrdiff_t>,
               std::format("U::difference_type should be std::ptrdiff_t, but is {}", typeid(typename U::difference_type).name()) };
    co_yield{ std::is_same_v<typename U::size_type, std::size_t>,
               std::format("U::size_type should be std::size_t, but is {}", typeid(typename U::size_type).name()) };
    co_yield{ std::is_same_v<typename U::propagate_on_container_copy_assignment, std::false_type>,
               std::format("U::propagate_on_container_copy_assignment should be std::false_type, but is {}", typeid(typename U::propagate_on_container_copy_assignment).name()) };
    co_yield{ std::is_same_v<typename U::propagate_on_container_move_assignment, std::true_type>,
               std::format("U::propagate_on_container_move_assignment should be std::true_type, but is {}", typeid(typename U::propagate_on_container_move_assignment).name()) };
    co_yield{ std::is_same_v<typename U::propagate_on_container_swap, std::false_type>,
               std::format("U::propagate_on_container_swap should be std::false_type, but is {}", typeid(typename U::propagate_on_container_swap).name()) };
    co_yield{ std::is_same_v<typename U::is_always_equal, std::true_type>,
              std::format("U::is_always_equal should be std::true_type, but is {}", typeid(typename U::is_always_equal).name()) };
    co_return;
}
